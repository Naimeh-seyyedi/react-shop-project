...
import WithSideBar from "layout/WithSideBar";
  ...
  function CategoryProduct() {
    ...
    const { data, loading, getPosts } = useFetch(
      `products?category_id=${categoryId}&_page=${activePage}&_limit=${limit.current}`
    );
  
    useEffect(() => {
      (async () => {
        let response = await ApiAdmin.products(`categories?id=${categoryId}`)
      ...
      <React.Fragment>
        ...
        {loading ? (
          ...
          <>
            ...
            <Grid item xs={12}>
              <Typography className="suggestCat" sx={{ marginTop: 8 }}>
